...
const FINAL_PEXELS_MAPPINGS = [
    { filename: 'metin_oktay_real.jpg', search: 'football player vintage turkey galatasaray' },
    { filename: 'presidential_flag_turkey.jpg', search: 'turkey flag presidential emblem' }
];
...
async function downloadFinalTwo() {
    console.log('🚀 KALAN 2 RESİM PEXELS\'TEN İNDİRİLİYOR...');
    console.log(`📊 Toplam: ${FINAL_PEXELS_MAPPINGS.length} dosya`);
    console.log('=' * 40);
    
    let successCount = 0;
    let errorCount = 0;
    
    for (const mapping of FINAL_PEXELS_MAPPINGS) {
        try {
    ...
    console.log(`📁 Toplam: ${FINAL_PEXELS_MAPPINGS.length}`);
}

downloadFinalTwo().catch(console.error);
